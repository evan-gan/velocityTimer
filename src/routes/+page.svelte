<script lang="ts">
	...
	let timeDiff = $derived(
		startMark !== null && endMark !== null ? Math.abs(endMark - startMark) : null
	);
	...
	let velocity = $derived(() => {
		if (timeDiff === null || !distance || parseFloat(distance) <= 0 || timeDiff === 0) return null;
		...
		<section class="review-section">
			<div class="video-container review-video-container">
				<video
					...
					ontimeupdate={() => {
						if (reviewVideoEl && !isDragging) currentTime = reviewVideoEl.currentTime;
					}}
					playsinline
					preload="auto"
				></video>
			...
			<div class="scrubber-section">
				...
				>
					...
					{#if startMark !== null && endMark !== null}
						<div
							class="marker-range"
							style="left: {(Math.min(startMark, endMark) / (duration || 1)) * 100}%; width: {(Math.abs(endMark - startMark) / (duration || 1)) * 100}%"
						></div>
					{/if}
					<div class="scrubber-fill" style="width: {(currentTime / (duration || 1)) * 100}%">
					</div>
					<div
			...
			<div class="frame-controls">
				<button class="btn btn-frame" onclick={() => stepFrame(-1)} title="Back 1 frame">
					◀ -1
				</button>
				<button class="btn btn-frame" onclick={() => stepFrame(1)} title="Forward 1 frame">
					+1 ▶
				</button>
			</div>
			...
			<div class="mark-controls">
				...
				<button class="btn btn-end-mark" onclick={setEndMark}>
					SET END
				</button>
				<button class="btn btn-clear" onclick={clearMarks}>
					CLEAR
				</button>
			</div>
			...
			<div class="velocity-section">
				<h2>VELOCITY {#if timeDiff !== null}• Δt = {timeDiff.toFixed(4)}s{/if}</h2>
			...
			<div class="new-recording">
				<button class="btn btn-new" onclick={resetAll}>
					NEW RECORDING
				</button>
			</div>
		</section>
